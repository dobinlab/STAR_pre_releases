...
void ReadAlign::outputAlignments()
{
    ...
    { // otherwise align is held for the 2nd stage of outFilterBySJout
        ////////////////////////////////////
        if (unmapType < 0)
        { // passed mappedFilter. Unmapped reads can have nTr>0
            ...
            { // transform genome only on 2nd pass
        ...
        if (P.quant.trSAM.yes && unmapType < 0)
        { // Aligned.toTranscriptome output, only for mapped
...
{ // junction output for mapped reads (i.e. passed BySJout filtering)
    ...
    if (P.outSJfilterReads == "All" || nTrO == 1)
    {
        ...
        for (uint64 iTr = 0; iTr < nTrO; iTr++)
        { // write all transcripts junctions
...
{                                // filtering by SJout
    ...
    { // check transcript for unannotated junctions
        for (uint iex = 0; iex < trMult[iTr]->nExons - 1; iex++)
        { // check all junctions
    ...
    if (!outFilterBySJoutPass)
    {                   // this read is held for further filtering BySJout, record fastq
...
void ReadAlign::writeSAM(uint64 nTrOutSAM, Transcript **trOutSAM, Transcript *trBestSAM)
{
    ...
    if (unmapType < 0 && outFilterBySJoutPass)
    { // write to SAM/BAM
        ...
        if (P.outSAMfilter.yes)
        {
            ...
            {
                for (uint itr = 0; itr < nTrOutSAM; itr++)
        ...
        { // write transcripts
            ...
            { // SAM output
                ...
                { // keep pairs && paired reads && one of the mates not mapped in this transcript //not readNends: this is alignment
                    outBAMbytes += outputTranscriptSAM(*(trOutSAM[iTr]), 0, 0, (uint)-1, (uint)-1, 0, 4, mateMapped1, outSAMstream);
                };
            };

            if (P.outBAMunsorted || P.outBAMcoord)
            { // BAM output
                ...
                { // unsorted
                    ...
                    { // output each mate //not readNends: this is alignment
        ...
        { // not readNends: this is alignment
            unmapType = 4;
        };

        if (unmapType == 4 && P.outSAMunmapped.within)
        { // output unmapped ends for single-end alignments of PE reads
            ...
            if (P.outBAMcoord || (P.outBAMunsorted && !P.outSAMunmapped.keepPairs))
            { // BAM output
                ...
                for (uint imate = 0; imate < P.readNmates; imate++)
                { // alignBAM output is empty for mapped mate, but still need to scan through it //not readNends: this is alignment
    ...
    else if (unmapType >= 0 && P.outSAMunmapped.within)
    { // output unmapped within && unmapped read && both mates unmapped
        if (P.outBAMcoord || P.outBAMunsorted || P.quant.trSAM.bamYes)
        { // BAM output
            alignBAM(*trBestSAM, 0, 0, mapGen.chrStart[trBestSAM->Chr], (uint)-1, (uint)-1, 0, unmapType, mateMapped, P.outSAMattrOrder, outBAMoneAlign, outBAMoneAlignNbytes);
            for (uint imate = 0; imate < P.readNmates; imate++)
            { // output each mate //not readNends: this is alignment
...
void ReadAlign::outReadsUnmapped()
{
    ...
    { // output to fasta/q files. Include unmapType==4, i.e. one-mate alignments of PE reads
        for (uint im = 0; im < P.readNends; im++)
        {
...
{ // temporary: SAM output for SpliceGraph
    ...
    { // convert to new genome
        nTrOutSAM = 0;
        for (uint32 iTr = 0; iTr < nTrOutSAM; iTr++)
        {                                                   // convert output transcripts into new genome
